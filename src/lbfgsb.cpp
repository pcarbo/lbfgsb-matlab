...
try {
	...
	if (lb != x || ub != x)
		...
		"Input arguments LB and UB must have the same structure as X");
	...
	while (k < nrhs) {
		...
		if (k < nrhs) {
			...
			if (!strcmp(optionLabel, "maxiter"))
				...
				pgtol = value;
			else if (!strcmp(optionLabel, "iprint"))
				iprint = value;
			else {
				if (iterFunc)
					delete iterFunc;
	...
	MatlabProgram program(x, lb, ub, &objFunc, &gradFunc, iterFunc,
			(mxArray*) auxData, m, maxiter, factr, pgtol, iprint);
	...
	if (exitStatus == abnormalTermination) {
		if (iterFunc)
			delete iterFunc;
		throw MatlabException(
				"Solver unable to satisfy convergence \
